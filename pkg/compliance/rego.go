...
func NewRegoRunner(resolver Resolver) *RegoRunner {
	return &RegoRunner{
		resolver: resolver,
	}
}

func (rr *RegoRunner) RunBenchmarkRule(ctx context.Context, benchmark *Benchmark, rule *Rule) []*CheckEvent {
	...
	if err != nil {
		...
		return []*CheckEvent{NewCheckError(RegoEvaluator, rule, benchmark, errReason)}
	}

	log.Infof("running rego check for rule=%s", rule.ID)
	events, err := rr.runRegoEvaluation(ctx, benchmark, rule, resolverOutcome)
	if err != nil {
...
func (rr *RegoRunner) runRegoEvaluation(ctx context.Context, benchmark *Benchmark, rule *Rule, resolverOutcome ResolverOutcome) ([]*CheckEvent, error) {
	...
	for _, data := range results {
		events = append(events, newCheckEventFromRegoResult(data, rule, resolverOutcome, benchmark))
	}
	return events, nil
}
