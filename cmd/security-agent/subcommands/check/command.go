...
func RunCheck(log log.Component, config config.Component, checkArgs *CliParams) error {
	...
	if checkArgs.file != "" {
		benchmarks, err = compliance.LoadBenchmarkFiles(compliance.LoadBenchmarkOptions{
			RootDir: filepath.Dir(checkArgs.file),
			Glob:    filepath.Base(checkArgs.file),
		})
		if err != nil {
	...
	for _, benchmark := range benchmarks {
		runner := compliance.NewRegoRunner(resolver)
		for _, rule := range benchmark.Rules {
			events := runner.RunBenchmarkRule(context.Background(), benchmark, rule)
